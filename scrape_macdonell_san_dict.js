...
const db = new sqlite3.Database( './data/dict/macdonell_san.sqlite' );

const BASE_URL = 'https://dsal.uchicago.edu'


const sqlTableCreate = `CREATE TABLE IF NOT EXISTS macdonell_san (
  key VARCHAR(100)  NOT NULL,
  lnum DECIMAL(10,3) ,
  data TEXT NOT NULL
);`

const sqlIndexCreate = `CREATE INDEX IF NOT EXISTS datum on macdonell_san(key);`

const sqlInsert = `INSERT INTO macdonell_san (key,lnum,data) VALUES (?,?,?);`
...
( async function main() {
  ...
  const initialPagePath = `/cgi-bin/app/macdonell_query.py?page=1`
...
async function scrapeDict( { initialPagePath } ) {
  ...
  while ( nextPath && idx < 5000 ) { // limit number of itirations, to avoid infinite loop
    ...
    if ( entryList?.length > 0 ) {
      ...
      // fs.writeFileSync( `./data/macdonell_${idx}.json`, contentRes )
...
async function scrapePage( { url, pageIndex } ) {
  ...
  // console.log( "🚀 ~ file: scrape_macdonell_sanl_dict.js:15 ~ main ~ res:", res )
  ...
  const entryList = entryDivs.map( ( idx, e ) => {
    ...
    const headEl = $( e ).children( 'hw' ).first().children( 'b' ).first()
    // console.log( headEl.text() )
    res.word = headEl.text()
    // console.log( $(e).html() )
    res.meaning = $( e ).html()
    res.meaning = res.meaning.replaceAll( '&nbsp;', '' ).trim()
